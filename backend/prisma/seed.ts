...
async function main() {
  ...
  // ─── 7. TourPoi ──────────────────────────────────────────────────────────────
  const approvedPois = createdPois.filter(
  ...
  for (const [idx, poi] of hcmPois.entries()) {
    const exists = await prisma.tourPoi.findFirst({
      where: { tourId: tourHCM.id, poiId: poi.id },
    });
    if (!exists) {
  ...
  for (const [idx, poi] of hoianPois.entries()) {
    const exists = await prisma.tourPoi.findFirst({
      where: { tourId: tourHoiAn.id, poiId: poi.id },
    });
    if (!exists) {
      await prisma.tourPoi.create({
        data: {
  ...
  const poiPlayCounts = [
    {
      poiName: "Quầy Bánh Mì Phượng Chính Gốc",
      plays: 45,
      avgDuration: 52,
      completionRate: 0.92,
    },
    {
      poiName: "Quầy Bún Bò Đặc Biệt",
      plays: 38,
      avgDuration: 48,
      completionRate: 0.88,
    },
    {
      poiName: "Góc Lịch Sử Bánh Mì Hội An",
      plays: 35,
      avgDuration: 45,
      completionRate: 0.85,
    },
    {
      poiName: "Góc Trưng Bày Gia Vị Truyền Thống",
      plays: 32,
      avgDuration: 42,
      completionRate: 0.82,
    },
    {
      poiName: "Góc Làm Topping Bánh Mì",
      plays: 28,
      avgDuration: 40,
      completionRate: 0.8,
    },
    {
      poiName: "Khu Vực Dùng Bữa & Trải Nghiệm",
      plays: 25,
      avgDuration: 38,
      completionRate: 0.78,
    },
    {
      poiName: "Xưởng Làm Bánh – Trải Nghiệm Trực Tiếp",
      plays: 22,
      avgDuration: 35,
      completionRate: 0.75,
    },
    {
      poiName: "Khu Vực Chế Biến Nước Dùng",
      plays: 20,
      avgDuration: 33,
      completionRate: 0.72,
    },
    {
      poiName: "Khu Vực Tạo Hình Bún Huế",
      plays: 18,
      avgDuration: 30,
      completionRate: 0.7,
    },
    {
      poiName: "Góc Bán Hàng & Bao Gì",
      plays: 15,
      avgDuration: 28,
      completionRate: 0.68,
    },
      ...
      await prisma.audioPlayHistory.create({
        data: {
          ...
          totalDurationSeconds:
            poiData.avgDuration + Math.floor(Math.random() * 10),
  ...
  console.log(
    `✅ Created ${createdSessions.length} user sessions with ${totalAudioPlays} audio plays`,
  );
