...
const ResetPasswordPage = () => {
  ...
  const onFinish = async ({ newPassword, confirmNewPassword }: { newPassword: string; confirmNewPassword: string }) => {
    ...
    try {
      ...
      navigate(ROUTES.auth.merchantLogin, { replace: true, state: { message: 'Password reset successfully! Please login.' } });
    } catch (err: unknown) {
  ...
  if (!token) {
    return (
      <AuthLayout>
        <Alert type="error" showIcon message="Invalid reset link" description="Please request a new password reset link." />
